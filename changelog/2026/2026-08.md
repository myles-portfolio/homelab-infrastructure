...
### Began Checkmk Phase 3 core infrastructure onboarding

Started production-class onboarding with the DNS service after completing low-risk Phase 2 validation.

Key changes:

* standardized Checkmk host identifiers using semantic environment-and-service naming rather than IP addresses
* recreated the development and DNS monitoring objects under the new naming standard while storing IP addresses separately
...
* validated agent communication and Linux service discovery after the container capability correction
* added an active DNS check targeted through production, core-infrastructure, and DNS-role classifications
* validated that a known internal DNS record resolves to its expected address through the monitored DNS server

The active DNS check establishes application-level service validation rather than relying only on host reachability or process state.

### Corrected DNS container startup delay

Investigated a DNS outage observed during the monitoring rollout and identified a network-readiness delay rather than a Pi-hole application failure.

The container used static IPv4 addressing while also requesting an unused DHCPv6 lease. The unresolved DHCPv6 request delayed network readiness for approximately five minutes, which delayed Pi-hole FTL startup because the service waits for the network-online target.

Key changes:

* removed the unused DHCPv6 setting from the container network configuration
* retained the existing static IPv4 configuration
* rebooted the container and confirmed prompt network initialization
* validated Pi-hole FTL, DNS listeners, and web service availability after restart

This correction reduced an avoidable recovery delay for a foundational infrastructure service.
